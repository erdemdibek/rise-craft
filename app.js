...
window.runSimulatorCalculation = function() {
    ...
    // Zorunlu kural: Ana reçete her zaman "Kendim Üreteceğim" (craft) gibi dallanmalı, pazar fiyatından etkilenmemeli!
    let testRaw = {}, testCraft = {};
    if (mainRecipe.materials) {
        Object.entries(mainRecipe.materials).forEach(([subMatId, count]) => {
            aggregateMaterials(subMatId, count, testRaw, testCraft);
        });
    }
    testCraft[recipeId] = 1; // Ana üretimi listeye dahil et

    let xpPerMainCraftCycle = dynamicXpPerCraft;
    Object.entries(testCraft).forEach(([cId, amount]) => {
        if (cId !== recipeId) { // Alt kademelerden gelen gizli XP bonusları
    ...
    if (mainRecipe.materials) {
        Object.entries(mainRecipe.materials).forEach(([subMatId, count]) => {
            aggregateMaterials(subMatId, count * mainCraftCount, finalRawGather, finalIntermediateCraft);
        });
    }
    finalIntermediateCraft[recipeId] = mainCraftCount; // Esas üretilecek miktar
    ...
    // KRİTİK DÜZELTME: Seçilen ana reçetenin kendisi (tüm meslekler için geçerli) pazar listesinden silinir!
    allUniqueMaterials.delete(recipeId);

    if (allUniqueMaterials.size === 0 && !mainRecipe.materials) { 
        matrixDiv.innerHTML = '<p class="text-xs text-gray-500 text-center py-2">Bu reçete için alt malzeme gerekmiyor.</p>'; 
        return; 
    }
    ...
    allUniqueMaterials.forEach(matId => {
        ...
        matrixHtml += `
            <div class="flex flex-col bg-black/40 border border-gray-900 p-2.5 rounded-xl space-y-2 animate-fadeIn">
    ...
    matrixDiv.innerHTML = matrixHtml || '<p class="text-xs text-gray-500 text-center py-2">Sadece temel ham maddeler gerekiyor.</p>';
    ...
    Object.entries(finalRawGather).forEach(([matId, amount]) => {
        ...
        totalCostGold += cost;
        rawListHtml += `<div class="flex justify-between text-gray-300">• <span>${matId.replace(/_/g, ' ').toUpperCase()}</span> <span class="font-bold text-white">${amount.toLocaleString()} Adet <span class="text-gray-500 text-[10px]">(${cost.toLocaleString()} G)</span></span></div>`;
    });
    ...
    resultPanel.innerHTML = `
        ...
        <div class="space-y-1 text-[11px]">
            ...
            <div class="flex justify-between border-b border-gray-900 pb-2 text-gray-400"><span>Hedeflenen Üretim Adedi:</span> <span class="text-amber-400 font-black text-xs">${mainCraftCount.toLocaleString()} Adet</span></div>
            
            <div class="pt-1.5 font-bold text-amber-400 text-[10px] uppercase tracking-wider">🛒 Pazar Alışveriş Listesi:</div>
            <div class="pl-1 space-y-0.5 max-h-[100px] overflow-y-auto custom-scrollbar">${rawListHtml || '<div class="text-gray-500">-</div>'}</div>
