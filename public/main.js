...
let deadBodies = [];
let phaserScene, playerCircle, playerSprites = {}, machineSprites = {}, nameTexts = {};
...
// Player pozisyonu artık server authoritative
...
function update() {
  ...
  if (joystick.dirX !== 0 || joystick.dirY !== 0) {
    const newX = playerCircle.x + joystick.dirX * playerSpeed * (1 / 60);
    const newY = playerCircle.y + joystick.dirY * playerSpeed * (1 / 60);
    socket.emit("movePlayer", { lobbyId, x: newX, y: newY });
  }

  // Diğer oyuncular ve makineler zaten server’dan güncelleniyor
  for (const id in playerSprites) {
    const t = nameTexts[id];
    t.setPosition(playerSprites[id].x, playerSprites[id].y - 30);
  }
