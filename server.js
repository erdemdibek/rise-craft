...
const machinePositions=[{x:200,y:200},{x:400,y:200},{x:600,y:200},{x:800,y:200},{x:1000,y:200},{x:200,y:800},{x:400,y:800},{x:600,y:800},{x:800,y:800},{x:1000,y:800}];
...
io.on("connection",socket=>{
  ...
  socket.on("playerInput",({lobbyId,dirX,dirY})=>{
    const l=lobbies[lobbyId];
    if(!l||!l.players[socket.id]) return;
  ...
  socket.on("killPlayer",({lobbyId,targetId})=>{
    ...
    const k=l.players[socket.id]; const t=l.players[targetId];
    if(!k||!t||!t.alive) return;
    const d=Math.hypot(k.x-t.x,k.y-t.y); if(d>80) return;
    t.alive=false;
  ...
  socket.on("repairMachine",({lobbyId,name})=>{
    ...
    m.state="ok"; io.to(lobbyId).emit("machineRepaired",{name});
  });
  ...
  socket.on("disconnect",()=>{ 
    for(const lobbyId in lobbies){
      if(lobbies[lobbyId].players[socket.id]){
        ...
        delete lobbies[lobbyId].ready[socket.id];
        delete lobbies[lobbyId].inputs[socket.id];
